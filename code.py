...
from config import code_font_size
...
# 生成高亮代码图
def highlight_image(code, language=None):
    if language is not None:
        lexer = get_lexer_by_name(language)
    else:
        lexer = guess_lexer(code)

    formatter = ImageFormatter(line_numbers=False, image_format='png', font_size=code_font_size, line_pad=15, style=OneDarkStyle)
